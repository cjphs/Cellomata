...
if (require.main === module) {
    ...
    if (t) {
        ...
        rules.addRule("V", new Clause("A", "always", "",-1,.5,0,0));
        rules.addRule("V", new Clause("D", "always", "",-1,1,0,0));

        // Dead -> Alive if 3*Alive nearby
        rules.addRule("D", new Clause("A","nearby", "A",-1,1,3,3));

        // Alive -> Alive if [2,3]*Alive nearby, Dead otherwise.
        rules.addRule("A", new Clause("A",  "nearby", "A",-1,1,2,3));
        rules.addRule("A", new Clause("D", "always", "",-1,1,0,0));

        let grid = new Grid(10,10,rules);

        // Evolution
        while(true) {
            console.log(grid.asString(column_delimiter=","));
            grid.evolve();
        }
    } else if (false) {
